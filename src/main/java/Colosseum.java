...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        //take input for hit points
        System.out.println("How many hit points will it have?");
        int hitPoints = userInput.nextInt();
        ...
        while (invalidPoint) {
            if (hitPoints <= MAX_HIT_POINTS && hitPoints >= 1) {
                ...
                hitPoints = userInput.nextInt();
            }
        }
        //take user input for level
        System.out.println("How Enter your attack level:");
        int attackLevel = userInput.nextInt();
        boolean invalidAttackLevel = true;
        while (invalidAttackLevel) {
            if (attackLevel >= 1 && attackLevel <= (hitPoints - 1)) {
                invalidAttackLevel = false;
            } else {
                System.out.println("Sorry. The attack level must be between 1 and " + (hitPoints - 1) + ":");
                attackLevel = userInput.nextInt();
            }
        }
        //find defense level
        int defenseLevel = hitPoints - attackLevel;
        //set values
    ...
    public static void determineWinner() {
        if (firstPokemon.hitPoints <= 0) {
            System.out.println(secondPokemon.name + " is the winner! (second)");
        } else if (secondPokemon.hitPoints <= 0) {
            System.out.println(firstPokemon.name + " is the winner! (first)");
        }
    }
